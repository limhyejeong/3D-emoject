...
const createEmoject = (mesh, category, activity, length, color) => {
    ...
    if (category == "anger") {
        ...
        noiseSettings = { 진폭: 10, 반경: 0.5 + activity * 0.1, 속도: 1 };
    } else if (category == "sadness") {
        geometry = new THREE.TorusGeometry(0.5, 0.3, 64, 64);
        noiseSettings = { 진폭: 0.5 + activity * 0.05, 반경: 0.5 + activity * 0.1, 속도: 1 };
    } else if (category == "disgust") {
        geometry = new THREE.ConeGeometry(1, 1.3, 128, 128);
        noiseSettings = { 진폭: 8 + activity * 0.5, 반경: 0.1 + activity * 0.05, 속도: 1 };
        ...
        noiseSettings = { 진폭: 0.1 + activity * 0.3, 반경: activity * 0.2, 속도: 1 };
    } else if (category == "trust") {
        geometry = new THREE.TorusKnotGeometry(0.6, 0.2, 128, 16, 2, 3);
        noiseSettings = { 진폭: 1 + activity * 0.1, 반경: 0.2 + activity * 0.05, 속도: 1 };
    } else if (category == "joy") {
        geometry = new THREE.TorusKnotGeometry(0.6, 0.1, 256, 16, 1, 5);
        noiseSettings = { 진폭: 1 * activity / 1.5, 반경: 0.1 * activity, 속도: 1 };
    }

    console.log(0.1 + activity * 0.2);
    console.log(activity * 0.1);

    if (length == 2) {
        matcapTexture = texLoader.load("./assets/textures/matcap/거친.png");
    } else if (length == 3) {
        matcapTexture = texLoader.load("./assets/textures/matcap/금구슬.png");
    } else if (length == 4) {
        matcapTexture = texLoader.load("./assets/textures/matcap/노랑은은.jpg");
    } else if (length == 5) {
        matcapTexture = texLoader.load("./assets/textures/matcap/마법구슬.png");
    } else if (length == 6) {
        matcapTexture = texLoader.load("./assets/textures/matcap/말랑.png");
    } else if (length == 7) {
        matcapTexture = texLoader.load("./assets/textures/matcap/무지개.png");
    } else if (length == 8) {
        matcapTexture = texLoader.load("./assets/textures/matcap/부들.png");
    } else if (length == 9) {
        matcapTexture = texLoader.load("./assets/textures/matcap/크리스탈.jpg");
    } else if (length == 10) {
        matcapTexture = texLoader.load("./assets/textures/matcap/갤럭시.png");
    }
